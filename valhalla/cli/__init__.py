...
def fix_index(table, value):
    models.db.engine.execute(
        f"ALTER SEQUENCE {table.__tablename__}_id_seq RESTART WITH {value};"
    )
    ...
    def migrate_data(old_db):
        ...
        for u in migrate(old_session.query(old_models.User), "Users"):
            ...
            models.db.session.add(
                models.User(id=u.id, uuid=UUID(u.uuid), name=u.name, address="0.0.0.0")
            )
            ...
            meta = (
                {
                    m.key: m.value
                    for m in (
                        old_session.query(old_models.Metadata).get(i)
                        for i in tex.metadata_.split("|")[1:-1]
                    )
                }
                if tex.metadata_
                else {}
